...
def fetch_private_ip_from_route53(hostname, zone_id):
    ...
    ip_address = route53.list_resource_record_sets(
        HostedZoneId=zone_id, StartRecordName=hostname, StartRecordType="A", MaxItems="1"
...
def fetch_tag_metadata(asg_name):
    ...
    tag_value = autoscaling.describe_tags(
        Filters=[{"Name": "auto-scaling-group", "Values": [asg_name]}, {"Name": "key", "Values": [HOSTNAME_TAG_NAME]}],
        MaxRecords=1,
    )["Tags"][0]["Value"]
...
def process_message(message):
    try:
        ...
        logger.info("Failed to process event. No LifecycleTransition key. Skipping. Detailed error: %s".format(str(e)))
        return
